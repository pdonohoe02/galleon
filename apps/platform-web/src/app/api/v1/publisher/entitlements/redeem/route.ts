...
export async function POST(request: Request) {
  ...
  try {
    ...
    const fields = ["entitlement_token", "resource_id", "publisher_origin", "redemption_nonce", "publisher_session_hash"] as const;
    if (fields.some((field) => typeof body[field] !== "string")) {
      return NextResponse.json(
        { ok: false, error: { code: "REQUEST_INVALID", message: "The redemption request is invalid.", retryable: false, request_id: crypto.randomUUID() } },
        { status: 400 },
      );
    }
    return NextResponse.json(await galleon.redeemEntitlement({
